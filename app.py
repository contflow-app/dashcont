...
# Garantir que a coluna com os rótulos existe e renomeá-la corretamente
if 'Rótulos de Linha' in df_detalhado.columns:
    ...
    st.error("Coluna 'Rótulos de Linha' não encontrada na Planilha2.")
df_detalhado = df_detalhado.iloc[:, :-1]  # Remove "Total Geral"
df_detalhado = df_detalhado.apply(pd.to_numeric, errors="coerce")
df_detalhado = df_detalhado.loc[:, ~df_detalhado.columns.str.contains("^Unnamed")]
...
df_detalhado.columns = df_detalhado.columns.astype(str).str.strip()

# Filtra colunas válidas com pelo menos um valor diferente de 0
colunas_validas = [col for col in df_detalhado.columns if df_detalhado[col].abs().sum() > 0]

if colunas_validas:
    mes_despesas = st.selectbox("Escolha o mês para ver as despesas:", colunas_validas)
    if mes_despesas in df_detalhado.columns:
        data = df_detalhado[mes_despesas].dropna().abs()
        if not data.empty:
            st.write("Prévia dos dados:", data.to_frame(name="Valor"))
            ...
            st.info("Não há dados disponíveis para este mês.")
else:
    st.warning("Nenhum mês com despesas encontrado.")
